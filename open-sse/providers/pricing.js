...
export function getPricingForModel(provider, model) {
  ...
  // 1. Provider-specific override — built-in first, then packages/providers/registry/ extension
  if (provider && PROVIDER_PRICING[provider]?.[model]) {
    return PROVIDER_PRICING[provider][model];
  }

  // 1.1. Provider-specific override from EXTRA_PROVIDER_PRICING (for packages/providers/registry/ extension)
  if (provider && EXTRA_PROVIDER_PRICING[provider]?.[model]) {

...
- Đã commit + push lên `opcode/quyit-02` (`0b1c0909`), user review xong — **plan này coi như hoàn tất**.

### Follow-up sau khi review/deploy (2026-09-22)

- **Bug phát hiện sau deploy:** trang `/dashboard/error-stats` crash ngay khi mở (React error #130 — "element type is invalid"). Nguyên nhân: [packages/components/index.js](../packages/components/index.js) re-export `ErrorFrequencyChart` như named export (`export { ErrorFrequencyChart } from ...`) trong khi component chỉ có `export default` → barrel import trả về `undefined`. Fix: đổi thành `export { default as ErrorFrequencyChart } from ...` (commit `d0f2aa0c`).
- **Cải thiện ngoài scope, phát sinh trong lúc điều tra:** log noise trên docker (`TOKEN_REFRESH`/`BG_TOKEN_REFRESH` chạy nền liên tục) làm khó theo dõi log request/lỗi thật. Thử approach ẩn riêng theo tag trước (commit `19b90f3e`, sau đó revert), rồi chốt lại rule đơn giản hơn ở đúng 1 chỗ: [src/lib/consoleLogBuffer.js](../src/lib/consoleLogBuffer.js) — chỉ `console.debug` mới in ra stdout thật (docker logs), còn `log/info/warn/error` chỉ hiển thị ở trang Console Log trong dashboard (commit `8444a3b6`). Không thuộc scope gốc của tính năng Error Stats, nhưng ghi lại đây vì cùng phát sinh trong buổi làm việc này.

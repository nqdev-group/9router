...
- `cmem/` — Context Memory Engine. Own `package.json` (`type:module`, `exports` map for `.`/`./core`/`./capture`/`./injection`/`./config`). `core/` (CmemEngine orchestrator, memoryStore, summarizer, contextBuilder, tokenBudget, configResolver), `capture/` (Observer), `injection/` (Injector + `formatters/{openai,claude,gemini}.js`), `utils/`, `config/defaults.js`. Root `index.js` barrels only `CmemEngine`. See `packages/cmem/AGENTS.md` for internals — including the DB-side detail that `cmemRepo.js`'s `initCmemTables()` is dead code; the real DDL lives in `MemoryStore.init()`.
- `components/` — Dashboard React UI, named exports only. Subfolders `caveman/`, `cmem/`, `cost/`, `rtk/`, `swagger/`, `token-saver-report/` each have their own local `index.js` barrel, but the root `index.js` re-exports leaf files directly (`./rtk/IntensitySelector.js`), bypassing the subfolder barrels. Imports `@/shared/components/Card` and `@/shared/constants/providers` from `src/` — an accepted exception for shared UI atoms, not general license. `swagger/SwaggerUIEmbed.js` loads `swagger-ui-dist` from a CDN (no npm dependency added) and is shared by the public docs page (`src/app/docs/api/`) and the internal dashboard docs page (`src/app/(dashboard)/dashboard/api-docs/`) — see `plans/2026-09-10-ollama-api-swagger-planning.md`.
...
- `ollama-compat/` — pure translation logic for 9Router's outward-facing Ollama-compatible API (mounted at `/api/v1/ollama/api/*`, see root AGENTS.md packages table + `plans/2026-09-10-ollama-api-swagger-planning.md`). `requestTranslate.js` (chat + generate + embed request → OpenAI shape), `responseTranslate.js` (chat: OpenAI JSON/SSE → Ollama JSON/NDJSON, timing fields in nanoseconds per Ollama spec), `generateTranslate.js` (same idea for `/api/generate`, field is `response` not `message.content`, no tool_calls), `embedTranslate.js` (`/api/embed`, always non-streaming), `modelCatalog.js` (`/api/tags`) and `showResponse.js` (`/api/show`) both take an already-fetched models array/entry as a plain argument, do NOT import `buildModelsList` themselves, to avoid a packages/ → src/ dependency, `psResponse.js`, `versionInfo.js`, `notImplemented.js` (`create/copy/pull/push/delete` — 501, 9Router has no local model files to manage). `index.js` barrels all nine.

...
export {
  ...
  getTokenSaverStats, getTokenSaverChartData, getTokenSaverPerRequest,
...
export async function exportDb() {
  ...
  for (const r of db.all(`SELECT key, value FROM kv WHERE scope = 'modelsDevPricing'`)) out.modelsDevPricing[r.key] = parseJson(r.value);
...
export async function importDb(payload) {
  ...
  db.transaction(() => {
    ...
    db.run(`DELETE FROM kv WHERE scope IN ('modelAliases', 'customModels', 'mitmAlias', 'pricing', 'modelsDevPricing')`);
  ...
  // Models.dev pricing
  const md = payload.modelsDevPricing;
  if (md) {
    for (const [k, v] of Object.entries(md)) {
      db.run(`INSERT OR REPLACE INTO kv(scope, key, value) VALUES('modelsDevPricing', ?, ?)`, [k, stringifyJson(v || {})]);
    }
  }
  });

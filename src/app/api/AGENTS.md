...
- `v1/ollama/api/` — Ollama-compatible chat surface: `chat`, `generate`, `embed`, `tags`, `ps`, `show`, `version` implemented; `create`/`copy`/`pull`/`push`/`delete` are 501 stubs (no local model files to manage). Mounted at `/api/v1/ollama/api/*` on purpose (not root `/api/*`) so Ollama SDK/CLI clients that let you override the base host URL and append `/api/chat` themselves land here unchanged — see `plans/2026-09-10-ollama-api-swagger-planning.md` §2.1. Translation logic lives in `@9router/ollama-compat`; `tags/route.js` and `show/route.js` both reuse `buildModelsList` exported from `v1/models/route.js` so they reflect the real catalog, not a static stub; `embed/route.js` thin-wraps `@/sse/handlers/embeddings.js` the same way `chat/route.js` wraps `@/sse/handlers/chat.js`. The 5 stub routes (`create`/`copy`/`pull`/`push`/`delete`) intentionally skip the `requireApiKey` gate the other routes get for free via `handleChat`/`handleEmbeddings` — acceptable since they return a static no-op error with no data exposure, but inconsistent with the rest of the surface if that gate ever matters here. Deliberately separate from the pre-existing `v1/api/chat/route.js` (+ `open-sse/utils/ollamaTransform.js`) and from `/api/version/route.js` (app-update-check, unrelated shape) — neither was touched.
- `combos/`, `keys/`, `providers/`, `provider-nodes/`, `proxy-pools/`, `models/`, `models-dev/`, `model-token-limits/`, `media-providers/`, `settings/`, `usage/`, `pricing/`, `tags/`, `error-stats/` — dashboard CRUD, one dir per resource, `[id]` for item routes. `combos/using-model` (reverse-lookup, `?model=` query param — not a dynamic path segment, since model strings contain `/`) and `combos/remove-model` (bulk-remove + clears the model's in-memory cooldown per affected combo) back the `/dashboard/error-stats` page's model actions (added 2026-09-22, see `plans/2026-09-22-provider-model-error-stats-planning.md`). `error-stats/accounts` and `error-stats/models` are thin wrappers around `src/lib/errorLogDb/errorLogRepo.js` (separate `error-log.sqlite` DB, not the app DB).
...
- `docs/internal-openapi/` — serves `internal-swagger.json` (generated by `scripts/generate-internal-openapi.mjs`, "path + summary + auth requirement" tier, ~150 admin routes, no request/response body schemas — see `plans/2026-09-10-ollama-api-swagger-planning.md` §6/§11) to the gated dashboard page `src/app/(dashboard)/dashboard/api-docs/`. Every operation gets a Swagger UI tag via the generator's `TAG_GROUPS` map (keyed by the route's top-level dir, e.g. `settings`/`keys`/`providers` → `Dashboard CRUD`, `headroom`/`pxpipe`/`tunnel` → `Sidecar Processes`) — keep `TAG_GROUPS` in sync when adding a new top-level dir, or the generator logs a `WARNING: unmapped top-level dir(s)` and those paths fall back to tag `"Other"`. Public client-facing API is documented separately (unauthenticated) at `public/openapi/public-swagger.json` + `src/app/docs/api/`. Re-run the generator after adding/removing routes under `src/app/api/` (outside `v1/`/`v1beta/`) or it goes stale.

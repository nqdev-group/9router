...
- `docs/internal-openapi/` — serves `spec.json` (generated by `scripts/generate-internal-openapi.mjs`, "path + summary + auth requirement" tier, ~150 admin routes, no request/response body schemas — see `plans/2026-09-10-ollama-api-swagger-planning.md` §6/§11) to the gated dashboard page `src/app/(dashboard)/dashboard/api-docs/`. Public client-facing API is documented separately (unauthenticated) at `public/openapi/ollama-public.json` + `src/app/docs/api/`. Re-run the generator after adding/removing routes under `src/app/api/` (outside `v1/`/`v1beta/`) or it goes stale.

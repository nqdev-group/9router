              ...
              {filteredCombos.map((combo) => {
                ...
                return (
                  <button
                    ...
                    className={`
                      flex items-center gap-3 px-3 py-2 rounded-lg text-xs transition-all text-left w-full
                      ${isAdded
                        ? "bg-primary/5 text-text-main"
                  ...
                  >
                    {isAdded ? (
                      ...
                      <span className="size-4 rounded-full border-2 border-text-muted/30 shrink-0" />
                    )}
                    <span className="font-mono flex-1 min-w-0 truncate">{combo.name}</span>
                    <span className="text-[10px] text-text-muted shrink-0">combo</span>
                  </button>
                );
              })}
            </div>
                ...
                return (
                  <button
                    ...
                    className={`
                      flex items-center gap-3 px-3 py-2 rounded-lg text-xs transition-all text-left w-full
                      ${isPlaceholder
                        ? "border border-dashed border-border text-text-muted hover:border-primary/50 hover:text-primary"
                        : isSelected
                  ...
                  >
                    {/* State indicator */}
                    {isPlaceholder ? (
                      <span className="material-symbols-outlined text-[14px] text-text-muted shrink-0">edit</span>
                    ) : isSelected || isAdded ? (
                      <span className={`material-symbols-outlined text-[14px] shrink-0 ${isSelected ? 'text-white' : 'text-primary'}`}>
                        check_circle
                      </span>
                    ) : (
                      <span className="size-4 rounded-full border-2 border-text-muted/30 shrink-0" />
                    ...
                    <span className="font-mono flex-1 min-w-0 truncate">
                      {isPlaceholder ? (
                        <span className="italic">{model.name}</span>
                      ) : (
                        <><span className="opacity-50">{group.alias}/</span>{model.name}</>
                      )}
                    </span>
                    ...
                    {!isPlaceholder && (
                      <span className="flex items-center gap-1.5 shrink-0">
                        {model.isCustom && (
                          <span className="text-[9px] px-1 py-0.5 rounded bg-primary/10 text-primary font-medium leading-none">custom</span>
                        )}
